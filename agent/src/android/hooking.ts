...
export namespace hooking {
  ...
  export const searchVarValue = (fqClazz: string, varFilter: string, filterOverload: string | null): Promise<void> => {
    const [clazz, method] = splitClassMethod(fqClazz);
    send(`Attempting to watch class ${c.green(clazz)} and method ${c.green(method)}.`);
    ...
    return wrapJavaPerform(() => {
      ...
      targetClass[method].overloads.forEach((m: any) => {
        ...
        m.implementation = function() {
          ...
          // Check if value in arguments
          if (calleeArgTypes.length > 0) {
            const argValues: string[] = [];
            for (const h of arguments) {
              if ( (h || "(none)").toString().includes(varFilter))
                argValues.push((h || "(none)").toString());
            }
            send(c.blackBright(`Check Value: ${c.red(varFilter)}`));
          ...
          // Check return value
          if ((retVal || "(none)").toString().includes(varFilter)){
            send(c.blackBright(`[${job.identifier}] `) + `Return Value: ${c.red(retValStr)}`);
          }
          send(c.blackBright(`Check Value: ${c.red(retVal)}`));
